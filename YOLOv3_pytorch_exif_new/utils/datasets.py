...
# 修改,加载y的Exif信息进来
class LoadImagesAndLabelsAndExif:  # for training
    def __init__(self, path, batch_size=1, img_size=608, augment=False):
        with open(path, 'r') as file:
            self.img_files = file.readlines()
            self.img_files = [x.replace('\n', '') for x in self.img_files]
        ...
        # 修改
        self.exif_files = [x.replace('images', 'exifs').replace('.png', '.txt').replace('.jpg', '.txt')
                            for x in self.img_files]
    ...
    def __next__(self):
        ...
        # 修改
        img_all, labels_all, exifs_all, img_paths, img_shapes = [], [], [], [], []
        for index, files_index in enumerate(range(ia, ib)):
            img_path = self.img_files[self.shuffled_vector[files_index]]
            label_path = self.label_files[self.shuffled_vector[files_index]]

            # 修改
            exif_path = self.exif_files[self.shuffled_vector[files_index]]
            ...
            if os.path.isfile(exif_path):
                exif_info = np.loadtxt(exif_path, dtype=np.float32)

                # todo 是否需要reshape还要考虑
                # exif_info = exif_info.reshape((1, 1))

                exif_info = np.resize(exif_info, (1, int(self.img_size/32), int(self.img_size/32)))
                # exif_info1 = exif_info / 100
                # print(exif_info1)
                exif_info = (exif_info - 1450) / 10
                # print(exif_info)

                exifs_all.append(exif_info)
        ...
        # 修改,把exifs_all转为array才能转为tensor
        exifs_all = np.array(exifs_all)
        exifs_all = torch.from_numpy(exifs_all)

        labels_all = torch.from_numpy(np.concatenate(labels_all, 0))
        return torch.from_numpy(img_all), labels_all, exifs_all, img_paths, img_shapes
