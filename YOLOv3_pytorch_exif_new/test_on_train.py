...
def test_on_train(
    ...
    test_path = data_cfg_dict['train']  # 测试集位置test.txt
    ...
    for i, c in enumerate(load_classes(data_cfg_dict['names'])):
        if AP_accum_count[i]:
            ...
            with open('mAP_on_train.txt', 'a') as file:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='test_on_train.py')
    parser.add_argument('--batch-size', type=int, default=1, help='size of each image batch')
    parser.add_argument('--cfg', type=str, default='cfg/yolov3-1cls-spp.cfg', help='cfg file path')
