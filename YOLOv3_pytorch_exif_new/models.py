...
def create_modules(module_defs):
   ...
   构建网络模型
    ...
    for i, module_def in enumerate(module_defs):
        ...
        # 创建上采样层，这里由于nn.upsample报警告，所以自己创建了Upsample函数
        elif module_def['type'] == 'upsampleY':
            # upsample = nn.Upsample(scale_factor=int(module_def['stride']), mode='nearest')  # WARNING: 不推荐nn.Upsample这种方式
            upsampleY = Upsample(scale_factor=int(module_def['stride']))  # scale_factor：尺度缩放因子
            modules.add_module('upsampleY_%d' % i, upsampleY)
        ...
        # 修改
        # 创建addExif层：空层，具体功能在前向传播中实现
        elif module_def['type'] == 'addExif':
            # 重新计算,为了方便conv层构建
            filters = output_filters[-1] + 1
            modules.add_module('add_%d' % i, EmptyLayer())

        # 另一种方法,直接设置函数,类似upsample直接forward
        # elif module_def['type'] == 'addExif':
        #     # upsample = nn.Upsample(scale_factor=int(module_def['stride']), mode='nearest')  # WARNING: 不推荐nn.Upsample这种方式
        #     exif_layer = exif_forward(scale_factor=int(module_def['add']))  # scale_factor：尺度缩放因子
        #     modules.add_module('addExif_%d' % i, exif_layer)
...
class exif_forward(nn.Module):
    # 自定义了add_exif层,不然可能报错

    def __init__(self, scale_factor=1):
        super(exif_forward, self).__init__()
        self.scale_factor = scale_factor

    def forward(self, x, y_tensor):
        add_parameter = self.scale_factor
        if add_parameter == 1:
            # y_tensor = torch.full([13, 13, 1], float(y))
            x = torch.cat((x, y_tensor), 1)
        elif add_parameter == 2:
            # y_tensor = torch.full([26, 26, 1], float(y))
            x = torch.cat((x, y_tensor), 1)
        elif add_parameter == 4:
            # y_tensor = torch.full([52, 52, 1], float(y))
            x = torch.cat((x, y_tensor), 1)
        return x
...
class Darknet(nn.Module):
    ...
    def forward(self, x, y, var=None):     # 前向传播
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'upsample', 'maxpool']:
                # print('con的xshape前:', x.shape)
                x = module(x)
                # print('con的xshape后:', x.shape)
            # 'route' 'shortcut' 'yolo'层单独写的运算
            elif mtype == 'upsampleY':
                x = module(y)
            # 'route' 'shortcut' 'yolo'层单独写的运算
            ...
            # 修改,添加对于addExif的处理方法
            # 是在此处处理y输入reshape还是在别处处理?
            elif mtype == 'addExif':
                multi_add = int(module_def['add'])
                if multi_add == 1:
                    # y_tensor = torch.full([13, 13, 1], float(y))
                    # print('exif的yshape:', y.shape)
                    x = torch.cat((x, y), 1)
                    # print('exif的xshape:', x.shape)
                elif multi_add == 2:
                    # y_tensor = torch.full([26, 26, 1], float(y))
                    x = torch.cat((x, layer_outputs[-2]), 1)
                elif multi_add == 4:
                    # y_tensor = torch.full([52, 52, 1], float(y))
                    x = torch.cat((x, layer_outputs[-2]), 1)
                # x = module[0](x, y)
