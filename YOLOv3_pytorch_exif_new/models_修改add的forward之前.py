...
def create_modules(module_defs):
   ...
   构建网络模型
    ...
    for i, module_def in enumerate(module_defs):
        ...
        #     filters = output_filters[int(module_def['add'])]
        ...
        # 另一种方法,直接设置函数,类似upsample直接forward
            ...
            modules.add_module('upsample_%d' % i, exif_layer)
    ...
    def forward(self, x, y, var=None):     # 前向传播
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'upsample', 'maxpool']:
                print('测试输入x是否为单个1：', x.shape)
                x = module(x)
                print('测试输入x是否为单个2：', x.shape)
            # 'route' 'shortcut' 'yolo'层单独写的运算
            elif mtype == 'route':
                layer_i = [int(x) for x in module_def['layers'].split(',')]
                if len(layer_i) == 1:
                    print('测试输入x是否为单个3：', x.shape)
                    x = layer_outputs[layer_i[0]]
                    print('测试输入x是否为单个4：', x.shape)
                else:
            ...
            # 是在此处处理y输入reshape还是在别处处理?
                ...
                print('测试输入y是否为单个：', y)
                print('测试输入y是否为单个：', y.shape)

                print('测试输入x是否为单个：', x.shape)
