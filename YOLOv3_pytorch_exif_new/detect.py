...
def detect(
    ...
    if weights.endswith('.pt'):  # pytorch format
        if weights.endswith('best.pt') and not os.path.exists(weights):
            print('test?')
    ...
    classes = load_classes(parse_data_cfg('data/BIRDSAI.data')['names'])   # TODO 迁移学习记得改
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='cfg/yolov3-spp-1cls.cfg', help='cfg文件路径')
    parser.add_argument('--weights', type=str, default='weights/best.pt', help='权重文件路径')    # TODO 改
    parser.add_argument('--images', type=str, default='data/samples', help='path to images')
    parser.add_argument('--img-size', type=int, default=32 * 13, help='size of each image dimension')    #TODO 原来为32*13=416
